...
int main(int argc, char** argv)
{
  ...
  while( ! (all_cores_done) ){
    ...
    if (cycle % 256000000 == 0 && cycle > 0) { // every 64 ms we call the refresh anyway!    
      //printf("Resetting the banks \n");
      gct_reset(memsys->gct); // Doing it serially for now. Ideally it wont happen like this. // might need some includes 
      rcc_flush(memsys->rct);
    }     
    cycle+= CLOCK_INC_FACTOR;
    /*
    // Adding code for the refresh as this loop is run every cycle
    if (cycle % 256000000 == 0 && cycle>0) { // every 64 ms we call the refresh anyway!
        for (int i = 0; i < memsys->mainmem->num_banks; i++) { // TODO
            mgries_reset(memsys->mgries_t[i]); // Doing it serially for now. Ideally it wont happen like this. // might need some includes 
        }
    }
    */
  }
