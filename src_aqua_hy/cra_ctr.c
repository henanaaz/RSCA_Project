...
uns64  cra_ctr_read(CraCtr *m, DRAM* d, Addr rowAddr, uns64 in_cycle, uns64* read_ctrval){
  Flag retval = FALSE;
  uns delay = 0;
...
void  cra_ctr_write(CraCtr *m, DRAM* d, Addr rowAddr, uns64 in_cycle, uns64 write_ctrval){
  ...
  bool hit = ctrcache_access(m->ctr_cache, counter_index);
  if (hit) {
      ctrcache_mark_dirty(m->ctr_cache, counter_index); // Idk how i  will use this but to avoid run time issues giving it a retunr variable only in this scope.
  }
  if (!hit) {
      assert(0);
  }
...
uns64 rct_initialize(CraCtr* m, DRAM* d, Addr rowAddr, uns64 in_cycle) {
    uns64 delay = 0;
    ...
    for (uns64 i = counter_index; i < counter_index + 128;i++) { // Need to see if this 128 counters should be hard coded or not.
...
void rcc_flush(CraCtr* m) {
    for (uns64 i = 0; i < m->ctr_cache->sets * m->ctr_cache->assocs; i++) {
