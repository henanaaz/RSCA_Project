...
uns64       MEM_T_RP        = 44;  // Row Precharge Time (tRP in DDR4)
uns64       MEM_T_RAS       = 102; // Minimum delay bet. Activate and Precharge commands (atleast tRCD + tRP in DDR4)
uns64       MEM_T_RC        = MEM_T_RAS + MEM_T_RP; // RowCycle time or ACT-to-ACT delay (min time bet ACT to different rows in a bank).  or 146nano seconds
...
//-- AQUA Related --
uns64       cycles_for_read = MEM_T_ACT + MEM_T_CAS + (MEM_PAGESIZE / MEM_BANKS) * MEM_T_BURST; // 600
uns64       cycles_for_write= MEM_T_CAS + (MEM_PAGESIZE / MEM_BANKS) * MEM_T_BURST; // 556
uns64       MEM_T_ROW_MOV   = cycles_for_read + cycles_for_write;  // for moving one row to the quarantine area = 1156 or 1.156microseconds
uns64		RQA_SIZE 	= 64 * 0.001 * ( MEM_BANKS /( (MEM_T_RC * RH_THRESHOLD_ACT/2) + MEM_BANKS*MEM_T_ROW_MOV ));
// RQA_SIZE =  2048 / ((0.022*rh_threshold) + 18.496)
//----------------------
//
